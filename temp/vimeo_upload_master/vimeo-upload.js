...
(function(root, factory) {
    ...
    */
    ...
    me.prototype.upload = function() {
		...
		$.post( glob_cms_home_dir+"temp/php_request/vimeo.php", {
			task 		: "upload_ticket"
		})
		.done(function( data ){
			...
			if(data != ""){
                ...
                this_cache.url = response.upload_link_secure
                this_cache.user = response.user
                this_cache.ticket_id = response.ticket_id
                this_cache.complete_url = defaults.api_url + response.complete_uri
                this_cache.sendFile_()
            } else {
    ...
    me.prototype.complete_ = function(xhr) {
		...
		$.post( glob_cms_home_dir+"temp/php_request/vimeo.php", {
			...
			filename 		: file.name,
			filesize 		: file.size,
			kurs_id			: file.kurs_id
		})
		.done(function( data ){
			if(data != ""){
				this_cache.onComplete(data);
				
            } else {
				alert("complete_error");
